...
if(!button.on.click){button.on.click=function(){btn=button.name;if(config.fieldXtype){value=this.upX('kijs.gui.Window').down('field').value;if(!this.upX('kijs.gui.Window').down('field').validate()){return;}}
...
set fileExtensions(val){if(!kijs.isArray(val)){val=[val];}
this._fileExtensions=[];kijs.Array.each(val,function(type){if(type.charAt(0)!=='.'){type='.'+type;}
this._fileExtensions.push(type);},this);}
...
let acceptTypes=kijs.Array.concat(this._contentTypes,this._fileExtensions);if(acceptTypes.length>0){input.setAttribute('accept',acceptTypes.join(','));}
...
_checkMime(mime){let match=false;if(mime.type&&this._contentTypes.length>0){mime=mime.type.toLowerCase();let mimeParts=mime.split('/',2);kijs.Array.each(this._contentTypes,function(contentType){if(mime===contentType||contentType===mimeParts[0]+'/*'){match=true;}},this);}else{let extension=mime.name.split('.').pop();if(extension&&this._fileExtensions.length>0){kijs.Array.each(this._fileExtensions,function(ext){if(ext==='.'+extension){match=true;}},this);}}
return match;}
...
_uploadFiles(fileList){this._uploadResponses={};if(fileList){for(let i=0;i<fileList.length;i++){if(this._checkMime(fileList[i])){this._uploadFile(fileList[i]);}else{this.raiseEvent('failUpload',this,this._getFilename(fileList[i].name),fileList[i].type);}}}}
...
return'';}
destruct(){this._dropZones=null;this._contentTypes=null;this._contentTypes=null;this._currentUploadIds=null;this._directory=null;this._dropZones=null;this._fileExtensions=null;this._maxFilesize=null;this._multiple=null;this._sanitizeFilename=null;this._uploadId=null;this._uploadResponses=null;this._filenameHeader=null;this._pathnameHeader=null;this._validMediaTypes=null;super.destruct();}};kijs.gui.ApertureMask=class kijs_gui_ApertureMask extends kijs.Observable{constructor(config={}){super(false);this._targetEl=null;this._targetDom=null;this._animated=true;this._topDom=new kijs.gui.Dom({cls:['kijs-aperturemask','top']});this._rightDom=new kijs.gui.Dom({cls:['kijs-aperturemask','right']});this._leftDom=new kijs.gui.Dom({cls:['kijs-aperturemask','left']});this._bottomDom=new kijs.gui.Dom({cls:['kijs-aperturemask','bottom']});this._configMap={animated:true,cls:{fn:'function',target:this.clsAdd},target:{target:'target'}};if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}
...
set collapsed(val){if(val){if(val==='toggle'){if(this.collapsed){this.expand();}else{this.collapse();}}else{this.collapse();}}else{this.expand();}}
...
_validationRules(value){if(this._required){if(!value){this._errors.push(kijs.getText('Dieses Checkbox muss bestätigt werden'));}}}
...
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
if(this._inputDom){this._inputDom.destruct();}
this._inputDom=null;super.destruct(true);}};kijs.gui.field.QuillEditor=class kijs_gui_field_QuillEditor extends kijs.gui.field.Field{constructor(config={}){super(false);this._disabled=false;this._quillEditor=null;this._quillEditorNode=null;this._readOnly=false;this._theme='snow';this._value=null;this._toolbarOptions=[['bold','underline','strike'],['blockquote','code-block'],[{header:1},{header:2}],[{list:'ordered'},{list:'bullet'}],[{script:'sub'},{script:'super'}],[{indent:'-1'},{indent:'+1'}],[{direction:'rtl'}],[{size:['small',false,'large','huge']}],[{header:[1,2,3,4,5,6,false]}],[{color:[]},{background:[]}],[{font:[]}],[{align:[]}],['clean']];Object.assign(this._defaultConfig,{cls:'kijs-field-quilleditor'});Object.assign(this._configMap,{disabled:true,readOnly:true,theme:true,toolbarOptions:true});this.on('input',this._onInput,this);if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
get disabled(){return super.disabled;}
set disabled(val){this._disabled=val;super.disabled=!!val;if(val||this._dom.clsHas('kijs-disabled')){this._quillEditor.disable(true);}else{this._quillEditor.enable(true);}}
get isEmpty(){return kijs.isEmpty(this.value);}
get readOnly(){return this._readOnly;}
set readOnly(val){this._readOnly=val;super.readOnly=!!val;if(val||this._dom.clsHas('kijs-readonly')){this._quillEditor.disable(true);}else{this._quillEditor.enable(true);}}
get theme(){return this._theme;}
set theme(val){this._theme=val;}
get trimValue(){return this._trimValue;}
set trimValue(val){this._trimValue=val;}
...
_onInput(e){this.validate();}
destruct(superCall){if(!superCall){this.unrender(superCall);this.raiseEvent('destruct');}
this._disabled=false;this._readOnly=null;this._quillEditor=null;this._quillEditorNode=null;this._theme=null;this._toolbarOptions=null;this._value=null;super.destruct(true);}};kijs.gui.field.Text=class kijs_gui_field_Text extends kijs.gui.field.Field{constructor(config={}){super(false);this._inputDom=new kijs.gui.Dom({disableEscBubbeling:true,nodeTagName:'input',nodeAttribute:{id:this._inputId}});this._trimValue=true;this._dom.clsAdd('kijs-field-text');Object.assign(this._configMap,{trimValue:true,placeholder:{target:'placeholder'}});this._eventForwardsAdd('blur',this._inputDom);this._eventForwardsAdd('change',this._inputDom);this._eventForwardsAdd('focus',this._inputDom);this._eventForwardsAdd('input',this._inputDom);this._eventForwardsRemove('enterPress',this._dom);this._eventForwardsRemove('enterEscPress',this._dom);this._eventForwardsRemove('escPress',this._dom);this._eventForwardsAdd('enterPress',this._inputDom);this._eventForwardsAdd('enterEscPress',this._inputDom);this._eventForwardsAdd('escPress',this._inputDom);this.on('input',this._onInput,this);if(kijs.isObject(config)){config=Object.assign({},this._defaultConfig,config);this.applyConfig(config,true);}}
get disabled(){return super.disabled;}
